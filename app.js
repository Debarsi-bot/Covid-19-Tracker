...
//render the countries in form of a list
let countryDropdownString = "";
countryJSON.forEach(countryListObject => {
    let country = countryListObject.country;
    countryDropdownString += `<li><a class="dropdown-item country-list text-white" href="#">${country}</a></li>`;
})
...
//add click event listener to all countries in the list
...
countryList.forEach((item, index) => {
    item.addEventListener('click', ()=> {
        ...
        //change text from "Select Country" to the country selected
...
//some data is corrupted meaning, although count is cumulative, sum index have values lesser than previous index, which is impossible
//in that case we fix the data by setting count of that day equal to previous
...
const displayCountryHistoryData = function(data2){
    ...
    dailyDeathCounts = [0];
    //create daily death data by subtracting previous dates death
    for (let i=1; i<dates.length; i++)
        dailyDeathCounts.push(deathCounts[i]-deathCounts[i-1]);
    confirmedDates = dates;
    //arrange data for confiremed cases
    confirmedCounts = [];
...
const displayError = function(){
    cardCountryName.innerText = 'Could not fetch data';
    spinner.style.display = 'none';
}

//search button is clicked
//fetch data from api, create the chart and render to screen
